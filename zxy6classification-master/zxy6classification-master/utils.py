...
# def action_arr2id(arr):
#     """ Converts action from the array format to an id (ranging from 0 to n_actions) """
#     # Create a dictionary that maps actions to their ids
#     action_to_id = {tuple(a): i for i, a in enumerate(actions)}
#     # Look up the ids for each action in the input array
#     ids = [action_to_id[tuple(a)] for a in arr]
#     return np.array(ids)
...
#     first dead_start samples of every episode.
#     """
#     # Create a list of arrays to store the sliced arrays from arr
#     sliced_arrs = [np.array(arr[i][dead_start:], dtype=np.float32) for i in range(len(arr))]
#     # Concatenate the sliced arrays along the first axis (vertically) to form a single large array
#     stack = np.concatenate(sliced_arrs, axis=0)
#     return stack