...
class Classifier_From_Layers:
    ...
    def __init__(self, layers: List[Layer]):
        ...
        summ_train_acc = tf.compat.v1.summary.scalar('Training_accuracy', self.accuracy)
        self.summ_train = tf.compat.v1.summary.merge([summ_loss, summ_train_acc])
        self.summ_valid = tf.compat.v1.summary.scalar('Validation_accuracy', self.accuracy)
    ...
    def train(self, X_train, y_train, X_valid, y_valid, n_batches, batch_size, lr, display_step=100):
        ...
        writer = tf.compat.v1.summary.FileWriter('tensorboard_data/' + timestr, self.sess.graph)
        # Training loop:
        for step in range(n_batches):
            ...
            if step % display_step == 0:
                ...
                writer.add_summary(summ, step)
                print(f'Step: {step}, Loss: {loss:.5f}, Training_accuracy: {train_acc:.4f}, Validation_accuracy: {val_acc:.4f}')
        writer.close()
